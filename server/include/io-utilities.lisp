#|===== Module: io-utilities =========================================
 ...
 (file->string file-path state)
    delivers (mv string error state) where string contains all characters
    in the file
    Note: Directory separator in file-path string must be "/", not "\"
    Note: "Example functions" section shows how to use this function
    ...
    Warning! Reading very big files might exhaust virtual memory

 (rat->str r d)
     delivers string containing decimal representation of a rational
     number, r, with d decimal places digits following the decimal.

 (str->rat str)
    delivers rational number represented by str, a string of decimal 
    digits, possibly containing a decimal point, and possibly with a
    leading +/- sign

 (loc file-path state)
    Reports number of lines of code in file specified by file-path 
    (a string). Also reports state, which is annoying
====================================================================|#
...
#|===== Usage Note ===================================================
 ...
 To certify this module, open it in DrACuLa and press Save/Certify

 Warning! This book imports list-utilities
          Make sure a certified list-utilities book is in
          the same folder with this one

 To make definitions in this module available in a module residing
 in iEx0, iEx1, ..., put the following commands in that module:
 Note: The definitions in list-utilities also become available
       in the module that imports this one
   (include-book "../SE/io-utilities")
   (set-state-ok t)
====================================================================|#
...
#|===== Function: (dgts->int dgts) ===================================
  ...
  Delivers integer whose decimal digits comprise dgts 
...
#|===== Function: (str->int str) =====================================
  ...
  Post: (str->int str) is the integer specifed, in decimal notation, 
        in str
====================================================================|#
(defun str->int (str)
...
#|===== Function: (rat->str r d) =====================================
  ...
  Returns string containing decimal representation of r, to d decimal 
   places
...
#|===== (read-n-chars n li channel state) ============================
  ...
  Inserts up to n characters from channel into li, leaving the new 
  characters at the beginning of li in the reverse of the order 
  in they were received in
  ...
  Post: Up to n characters have been received from channel
        Returns multiple value containing list of chars, channel, 
	and state
====================================================================|#
...
#|===== (write-all-strings strli channel state) ======================
  ...
  (inserting a newline after the characters comprising each element 
  of strli)

  Pre : (and (string-listp strli)
             (character-input-channelp channel))
  Post: channel has received chars from strings in strli, plus 
        newlines. Returns multiple value containing channel and new 
	state
====================================================================|#
...
#|===== (file->string file-path state) ===============================
  ...
  Post: Characters in file become accessible as string portion
        ...
        Returns multiple value:
          ...
          (3) state
  Warning! System may run out of virtual memory when reading big files
...
(defun file->string (fname state)
   (mv-let (chli error state)
		   ...
		   (open-input-channel fname :character state)
              (if (null channel)
                  (mv nil
                      (string-append "Error opening file for input: "
                                     fname)
                      state)
...
#|===== (loc file-path state) ========================================
 ...
 WARNING! The input file must use Unix-style line-terminators.
          ...
          you must convert it to Unix style, which you can do like 
	     this: dos2unix "some-program.lisp"
          Of course, you'll need to convert the file back to Windows 
	  style (unix2dos) when you want to edit it
 Sample usage:
 First:
   ...
   copy "c:\ACL2-3.2\Sources\Books\SE\io-utilities.lisp" c:\
   dos2unix c:\io-utilities.lisp
 Then, in an ACL2 session:
   (include-book "io-utilities")
   (set-state-ok t)
   :comp read-n-chars ; Warning! This cmd is for sessions only, 
                        not books
...
(defun num-noncomments (lines count)
   (if (not (consp lines))
       ...
       (let* ((whitespace '(#\Space #\Newline #\Tab))
              (leading-white-stripped (drop-set whitespace 
			                        (car lines))))
...
;                     (Well, it's "C:\foo.wpd" and "C:\bar.wpd"
;                     but ACL2 uses Unix conventions in path names)

(defun rwfoobar (state)
  (mv-let (input-as-string error-open state) 
	  (file->string "C:/foo.wpd" state)
     (if error-open
         (mv error-open state)
...
;                               ;   each element in a list in which 
;                               ;   each
...
; (your-computation str) ; This formula converts a string to a list 
                         ;   strings each of which is a substring of 
			 ;   str filling in the gap between two 
			 ;   newline-characters
			 ...
			 ;   program to do, and then it would 
			 ;   represent the results of th comptutation
			 ;   as a list of strings

(defun your-computation (str)
  (map-chrs->str (packets #\Newline (str->chrs str))))
  
; (ipo f-in f-out state) ; This formula uses the function file->string
                         ;   reads the file f-in, producing a string 
			 ;   containing all the characters in the file
                         ; Then it invokes the function 
                         ;   "your-computation" to transform the 
			 ;   string to a list of strings
                         ; Finally, it writes the list of strings
                         ;   delivered by your-computation to a file 
                         ;   named f-out, one string per line
                         ; USING THIS INPUT/PROCESS/OUTPUT TEMPLATE
                         ;   Copy the defun for ipo,
                         ;   change its name to something appropriate,
                         ;   define a new function to do the
                         ;   computation you are working on (note: 
                         ;   your the input to your function will be a
                         ;   of all the characters from your input 
                         ;   file, string consisting and the output 
                         ;   must be  a list of strings, one string 
                         ;   for each line you want in your output 
                         ;   file), and, finally, replace the 
                         ;   invocation of "your-computation" by an 
			 ;   invoction of the function you defined
                         ; Warning! The input file f-in must be a 
                         ;   text-file with Unix-style line separators
                         ;   (You can convert DOS-style files to 
                         ;   Unix-style with the dos2unix program.)
                         ; Warning! The strings f-in and f-out must be
                         ;   Unix-style file-path specs, with forward 
                         ;   slashes not Windows-style back slashes
                         ;   For example, "C:/yourfolder/yourfile.wpd"
                         ;            not "C:\yourfolder\yourfile.wpd"
                         ; Warning! The third parameter of ipo cannot 
                         ;   be any symbol other than the symbol whose
                         ;    name is "state"
                         ; Warning! Before invoking this function from
                         ;   an ACL2 session, you must issue these 
			 ;   commands:
                         ;   (set-state-ok t)
                         ;   :comp read-n-chars
